...
class YouTubeWorkflow:
    ...
    def __init__(self, notifier: Optional[Notifier] = None, retention_policy: ArtifactRetentionPolicy | None = None):
        self.run_id = None
        self.mode = "daily"
        self.context: Optional[WorkflowContext] = None
        self._log_session = get_log_session()
        self.notifier: Notifier = notifier or discord_notifier
        self.retention_policy = retention_policy or ArtifactRetentionPolicy()
    ...
    async def execute_full_workflow(self, mode: str = "daily") -> Dict[str, Any]:
        ...
        while attempt < max_attempts:
            ...
            for index in range(start_index, len(self.steps)):
                ...
                if result.artifacts:
                    self.context.add_artifacts(result.artifacts)
    ...
    def _compile_final_result(self, step_results: List[Any], execution_time: float) -> Dict[str, Any]:
        ...
        artifacts = list(self.context.artifacts) if self.context else []
        artifact_payload = [artifact.to_payload() for artifact in artifacts]
        generated_files = [payload["path"] for payload in artifact_payload]
        ...
        workflow_result = WorkflowResult(
            ...
            generated_artifacts=artifact_payload,
    ...
    def _cleanup_temp_files(self):
        ...
        for artifact in self.context.artifacts:
            ...
            if not path or path in processed:
                continue
            processed.add(path)
            if not self.retention_policy.should_delete(artifact):
                retained_count += 1
                continue
            try:
        ...
        if retained_count > 0:
            logger.info(f"Retained {retained_count} persisted artifacts")
