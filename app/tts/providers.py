...
class VoicevoxProvider(TTSProvider):
    ...
    def _server_is_healthy(self) -> bool:
        try:
            health_response = requests.get(f"http://localhost:{self.port}/speakers", timeout=3)
        except RequestException as exc:
...
class Pyttsx3Provider(TTSProvider):
    ...
    """
    def __init__(self):
        super().__init__(next_provider=None)
        self.engine = None
        self.speaker_params = {
            "武宏": {"rate": 140, "volume": 0.9},
            "つむぎ": {"rate": 160, "volume": 0.95},
            "ナレーター": {"rate": 150, "volume": 0.9},
